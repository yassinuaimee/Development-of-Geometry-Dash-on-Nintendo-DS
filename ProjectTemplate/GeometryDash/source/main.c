...
int main(void) {
		...
		BGCTRL[3]=BG_COLOR_256 | BG_MAP_BASE(10) | BG_TILE_BASE(4) | BG_32x32;
		// map configuration
		BGCTRL[2]=BG_COLOR_256 | BG_MAP_BASE(0) | BG_TILE_BASE(1) | BG_64x32;
		...
		swiCopy(&ResRedBGMap[0], BG_MAP_RAM(10), 32*24);
		...
		dmaCopy(empty,&BG_TILE_RAM(1)[0],64);//0
		dmaCopy(full,&BG_TILE_RAM(1)[32],64);//1
		dmaCopy(ld,&BG_TILE_RAM(1)[64],64);//2
		dmaCopy(lm,&BG_TILE_RAM(1)[96],64);//3
		dmaCopy(um,&BG_TILE_RAM(1)[128],64);//4
		dmaCopy(rm,&BG_TILE_RAM(1)[160],64);//5
		dmaCopy(rd,&BG_TILE_RAM(1)[192],64);//6
		...
		for(row=0; row<24;row++){
			for(col=0;col<32;col++){
				...
				if(row==14){	//top of the triangle
					if(col==13 || col==25){
						BG_MAP_RAM(0)[row*32+col] = 4;
					}
				}
				else if(row==15){ //center of traingle
					if(col==12 || col==24){
						BG_MAP_RAM(0)[row*32+col] = 3;
						BG_MAP_RAM(0)[row*32+col+1] = 1;
						BG_MAP_RAM(0)[row*32+col+2] = 5;
					}
				}
				else if(row==16 ){ //bottom of triangle
					if(col==12 || col==24){
						BG_MAP_RAM(0)[row*32+col] = 2;
						BG_MAP_RAM(0)[row*32+col+1] = 1;
						BG_MAP_RAM(0)[row*32+col+2] = 6;
					}
				}
				else if(row>16){ //horizontal bar at the bottom
					BG_MAP_RAM(0)[row*32+col] = 1;
				}
				else{ //assigning anything else to transparent
					BG_MAP_RAM(0)[row*32+col] = 0;
				}
